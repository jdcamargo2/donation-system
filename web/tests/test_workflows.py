...
class MvpWorkflowRegressionTests(TestCase):
    ...
    def test_user_can_create_institution_project_donation_and_see_dashboard_update(self):
        ...
        self.assertNotContains(donation_form, 'name="status"')
        ...
        amount_classes = donation_form.context['form'].fields['amount'].widget.attrs['class'].split()
        self.assertIn('money-input', amount_classes)
        self.assertIn('js-money-input', amount_classes)
        self.assertIn('form-control', amount_classes)
        self.assertContains(donation_form, 'inputmode="decimal"')
